...
into #t22 from #t11 order by #t11.tovar 

select * from #t22
/*
if (select SUM(quantity1) quantity1,tovar from #t22 group by tovar)<(select sum(quantity) quantity,tovar from #t22 where id=1)
begin
 RAISERROR ('Количество товара на складе меньше чем запрашиваемое количество.', -- Message text.
               16, -- Severity.
               1 -- State.
               );
return
...
SELECT 
		 /*id,#t22.dat,#t22.parcel,#t22.quantity,*/#t22.quantity1,#t22.tovar  ---,#t22.lot
from #t22 group by #t22.quantity1,#t22.tovar order by #t22.tovar
OPEN vendor_cursor
...
set @a=(select top 1 id from #t22 where tovar=@tovar order by dat)
while (@a<=(select MAX(id) from #t22))

begin
select @l=quantity-@quantity1 from #t22 where id=@a and tovar=@tovar
if @l>=0
begin
insert into #t31 --values(@dat,@parcel,@quantity1,@tovar,@lot)
select DAT,parcel,@quantity1,tovar,lot from #t22 where id=@a and tovar=@tovar
update #t22 set quantity=quantity-@quantity1,quantity1=0  where id=@a and tovar=@tovar
--update #t22 set quantity1=0  where id=@a  --@id
break
end
if (@l<0 and (select  quantity from #t22 where id=@a and tovar=@tovar)>0)
begin
insert into #t31
select dat,parcel,quantity,tovar,lot from #t22 where id=@a and tovar=@tovar
select @quantity1=@quantity1-(select quantity from #t22 where id=@a and tovar=@tovar)
update #t22 set quantity=0 where id=@a and tovar=@tovar
...
begin
select @l1=quantity-@quantity11 from #t2 where id=@a1
if @l1>=0
begin
...
begin
insert into #t3
select dat,parcel,quantity,tovar,lot from #t2 where id=@a1
...
--update #t2 set quantity=@quantity0-@quantity11 where id=@id1
update #t2 set quantity1=0 where id=@id1
insert into #t3
select dat,parcel,@quantity11,tovar,lot from #t2 where id=@id1
